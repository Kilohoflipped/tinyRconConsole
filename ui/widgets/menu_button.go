...
const (
	ButtonContentStart  = 0
	ButtonContentMiddle = 1
	ButtonContentEnd    = 2
)
...
func NewMenuButtons(clickable *widget.Clickable, content *MenuButtonContent, style *MenuButtonStyle) *MenuButton {
	b := &MenuButton{
		Clickable:   clickable,
		Content:     content,
		IsNavigated: false,
...
func (b *MenuButton) Layout(gtx LC, theme THM) LD {
		...
		return layout.Flex{Axis: layout.Vertical, Alignment: layout.Middle}.Layout(gtx,
			layout.Rigid(func(gtx LC) LD {
				...
				if b.IsNavigated {
					...
					l.Color = Disabled(b.Style.TextColor)
				}
			...
			return layout.Flex{Axis: layout.Vertical, Alignment: layout.Middle}.Layout(gtx,
				layout.Rigid(func(gtx LC) LD {
					...
					if b.IsNavigated {
						...
						iconColor = Disabled(b.Style.TextColor)
					}
	...
	btnBGLayout := func(gtx LC) LD {
		...
		if b.IsNavigated || b.Clickable.Hovered() {
			background = Hovered(b.Style.BgColor)
		} else {
			background = Disabled(b.Style.BgColor)
		}
